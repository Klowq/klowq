...
function WebLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (headerRef.current) {
      ...
      if (subHeaderRef.current) {
        document.documentElement.style.setProperty('--header-subheight', `${subHeaderRef.current.offsetHeight}px`);
      }
    }
  }, [mobileMenuOpen]);
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <nav ref={headerRef} className="border-b border-slate-200/60 bg-white/95 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden lg:flex items-center space-x-12">
              <Link href="/#features" className="text-slate-600 hover:text-slate-900 transition-colors font-medium tracking-wide">Platform</Link>
              <Link href="/#how-it-works" className="text-slate-600 hover:text-slate-900 transition-colors font-medium tracking-wide">How It Works</Link>
              <Link href="/#mobile-app" className="text-slate-600 hover:text-slate-900 transition-colors font-medium tracking-wide">Mobile App</Link>
              <Link href="/#faq" className="text-slate-600 hover:text-slate-900 transition-colors font-medium tracking-wide">FAQ</Link>
            </div>
            <div className="flex items-center space-x-4">
              <Button onClick={() => setshowWaitModal(true)} className="bg-primary hover:bg-primary/90 text-white shadow-lg hover:shadow-xl transition-all duration-300 px-8 py-3 rounded-xl">
                Join WaitList
                <BsArrowRight className="w-4 h-4 ml-2" />
              </Button>
              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 text-slate-600 hover:text-slate-900 rounded-lg hover:bg-slate-100 transition-colors">
        ...
        {mobileMenuOpen && (
          <div ref={subHeaderRef} className="lg:hidden border-t border-slate-200/60 bg-white/95 backdrop-blur-md">
      ...
      <footer className="bg-secondary/70 border-t border-slate-200/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-10 md:gap-16">
            <div className="md:col-span-2 space-y-6">
              ...
              <p className="text-slate-600 leading-relaxed max-w-md md:text-lg">
                Revolutionizing healthcare through anonymous consultations that connect patients with verified medical professionals while maintaining complete privacy and security.
              </p>
          ...
          <div className="border-t mt-5 border-slate-200/60 pt-8 text-center">
            <p className="text-slate-600 text-xs md:text-base">&copy; 2025 KLOWQ. All rights reserved. Connecting healthcare anonymously and securely.</p>
          </div>
        </div>
      </footer>
    </div>
